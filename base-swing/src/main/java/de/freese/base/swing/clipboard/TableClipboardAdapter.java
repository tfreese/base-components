...
public class TableClipboardAdapter extends AbstractClipboardAdapter
{
	 ...
	 * 
	 */
	private boolean externalPopup = false;
	...
	public void doPaste(final boolean flipAxes)
	{
		...
		if ((rowsSelected.length == 0) && (colsSelected.length == 0))
		{
			...
			JOptionPane.showMessageDialog(null, "Invalid Paste Selection",
					"Invalid Paste Selection", JOptionPane.ERROR_MESSAGE);
		...
		{
			clipboardString =
					(String) (getClipboard().getContents(this)
		...
		for (int row = 0; row < matrix.length; row++)
		{
			for (int col = 0; col < matrix[row].length; col++)
			{
				...
				if ((currentRow < getTable().getRowCount())
						&& (currentColumn < getTable().getColumnCount()))
				{
		...
		for (Point point : points)
		{
			...
			if ((getTable().getRowCount() > rowIndex)
					&& (getTable().getColumnCount() > columnIndex))
			{
