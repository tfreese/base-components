...
public class ColumnControlWindow implements AWTEventListener
{
	...
	public void eventDispatched(final AWTEvent event)
	{
		if ((event.getID() == MouseEvent.MOUSE_EXITED))
		{
			...
			if (!isChild(source, getWindow()) && !(event.getSource() instanceof JWindow)
					&& !event.getSource().equals(this.controlButton))
			{
				hideAndClear();
			}
		}
	...
	public void fill()
	{
		...
		{
			IExtTableColumnModel tableColumnModelExt =
					(IExtTableColumnModel) table.getColumnModel();
			...
			for (TableColumn tableColumn : columns)
			{
				...
				menuItem.addActionListener(new ActionListener()
				{
					...
					public void actionPerformed(final ActionEvent e)
					{
						...
						extTableColumn.setVisible(menuItem.isSelected());
					}
				});
		...
		// ZusatzActions
		String packText =
				Locale.GERMAN.equals(Locale.getDefault()) ? "Spalten anpassen" : "Pack Columns";
	...
	public void show()
	{
		...
		int dX =
				this.controlButton.getComponentOrientation().isLeftToRight() ? (-windowSize.width)
						: 0;
		...
		Toolkit.getDefaultToolkit().addAWTEventListener(ColumnControlWindow.this,
