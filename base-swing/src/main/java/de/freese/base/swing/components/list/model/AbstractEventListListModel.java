...
public abstract class AbstractEventListListModel<T> implements ListModel<T>, Serializable
{
	 ...
	 */
	public AbstractEventListListModel(final IEventList<T> list)
	{
		super();
	 ...
	 * elements of the list change. The changed elements are specified by the closed interval
	 * index0, index1 -- the endpoints are included. Note that index0 need not be less than or equal
	 * to index1.
	 * 
	 ...
	 * index1 -- the enpoints are included. Note that index0 need not be less than or equal to
	 * index1.
	 * 
	 * @param source the <code>ListModel</code> that changed, typically "this"
	 * @param index0 one end of the new interval
	...
	/**
	 * <code>AbstractListModel</code> subclasses must call this method <b>after</b> one or more
	 * elements are removed from the model. The new elements are specified by a closed interval
	 * index0, index1, i.e. the range that includes both index0 and index1. Note that index0 need
