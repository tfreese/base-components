...
public class ExtTableColumn extends TableColumn
{
	...
	/**
	 * Notifies registered <code>PropertyChangeListener</code>s about property changes. This method
	 * must be invoked internally whe any of the enhanced properties changed.
	 * <p>
	 * Implementation note: needed to replicate super functionality because super's field
	 * <code>propertyChangeSupport</code> and method <code>fireXX</code> are both private.
	 * 
	 ...
	 * <code>putClientProperty</code> will return a non-<code>null</code> value.
	 * 
	...
	{
		return ((key == null) || (this.clientProperties == null)) ? null : this.clientProperties
				.get(key);
	}
	...
	/**
	 * Sets the client property "key" to <code>value</code>. If <code>value</code> is
	 * <code>null</code> this method will remove the property. Changes to client properties are
	 * reported with <code>PropertyChange</code> events. The name of the property (for the sake of
	 * PropertyChange events) is <code>key.toString()</code>.
	 * <p>
	 * The <code>get/putClientProperty</code> methods provide access to a per-instance hashtable,
	 * which is intended for small scale extensions of TableColumn.
	 * <p>
	 * 
	 * @param key Object which is used as key to retrieve value
	...
	/**
	 * Sets the visible property. This property controls whether or not this view column is
	...
	public void setVisibleChange(final boolean visibleChange)
	{
		...
		firePropertyChange("visible_change", Boolean.valueOf(oldVisibleChange),
				Boolean.valueOf(visibleChange));
	}
}
