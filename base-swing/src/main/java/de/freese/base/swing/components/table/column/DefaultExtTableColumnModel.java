 ...
 */
public class DefaultExtTableColumnModel extends DefaultTableColumnModel implements
		IExtTableColumnModel
{
	/**
	 */
	...
	/**
	 * flag to distinguish a shown/hidden column from really added/removed columns during
	 * notification. This is brittle!
	 */
	...
	/**
	 * Listener attached to TableColumnExt instances to listen for changes to their visibility
	 * status, and to hide/show the column as oppropriate
	 */
	...
	public void addColumnModelListener(final TableColumnModelListener listener)
	{
		...
		{
			this.listenerList.add(IExtTableColumnModelListener.class,
					(IExtTableColumnModelListener) listener);
		}
	}

	/**
	 * Notifies <code>TableColumnModelExtListener</code>s about property changes of contained
	 * columns. The event instance is the original as fired by the <code>TableColumn</code>.
	 * 
	...
	/**
	 * hot fix for #157: listeners that are aware of the possible existence of invisible columns
	 * should check if the received columnAdded originated from moving a column from invisible to
	 * visible.
	 * 
	...
	public boolean isAddedFromInvisibleEvent(final int newIndex)
	{
		...
		return Boolean.TRUE.equals(((ExtTableColumn) getColumn(newIndex))
				.getClientProperty(IGNORE_EVENT));
	}

	/**
	 * hot fix for #157: listeners that are aware of the possible existence of invisible columns
	 * should check if the received columnRemoved originated from moving a column from visible to
	 * invisible.
	 * 
	...
	public boolean isRemovedToInvisibleEvent(final int oldIndex)
	{
		...
		return Boolean.TRUE.equals(((ExtTableColumn) this.currentColumns.get(oldIndex))
				.getClientProperty(IGNORE_EVENT));
	}
	...
	/**
	 * Update internal state after the visibility of the column was changed to invisible. The given
	 * column is assumed to be contained in this model.
	 * 
	...
	/**
	 * Update internal state after the visibility of the column was changed to visible. The given
	 * column is assumed to be contained in this model.
	 * 
	...
	public void removeColumnModelListener(final TableColumnModelListener listener)
	{
		...
		if (listener instanceof IExtTableColumnModelListener)
		{
			this.listenerList.remove(IExtTableColumnModelListener.class,
					(IExtTableColumnModelListener) listener);
		}
	}
