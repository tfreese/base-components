...
public abstract class AbstractClipboardAdapter
{
	 ...
	 * 
	 */
	private Action actionPasteFlipAxes = null;
	 ...
	 * Ist kein Konverter registriert wird der {@link ReflectionClipboardConverter} registriert und
	 * geliefert.
	 * 
	 * @param clazz Class
	...
	public ClipboardConverter getConverter(final Class<?> clazz)
	{
		...
		if (converter == null)
		{
			...
			getLogger().info("ReflectionClipboardConverter for {} registered.",
					clazz.getSimpleName());
		}
	...
	protected void initialize()
	{
		...
		getComponent().registerKeyboardAction(getActionCopy(), "region-copy", copyKS,
				JComponent.WHEN_FOCUSED);

		KeyStroke pasteKS = KeyStroke.getKeyStroke(KeyEvent.VK_V, ActionEvent.CTRL_MASK, false);

		getComponent().registerKeyboardAction(getActionPaste(), "region-paste", pasteKS,
				JComponent.WHEN_FOCUSED);
	}
