...
{
    /**
     * PropertyChangeListener des aktuell ausgeführten Tasks (ForegroundTask).
     *
     * @author Thomas Freese
     */
    private class TaskPCL implements PropertyChangeListener
    {
        ...
        public void propertyChange(final PropertyChangeEvent event)
        {
            ...
            if ((task != null) && (task == getForegroundTask()))
            {
                ...
                {
                    SwingWorker.StateValue newState = (SwingWorker.StateValue) event.getNewValue();

                    switch (newState)
                    {
                        case STARTED:
                            firePropertyChange(new PropertyChangeEvent(task, SwingTask.PROPERTY_STARTED, Boolean.FALSE, Boolean.TRUE));
                            break;
                        case DONE:
                            firePropertyChange(new PropertyChangeEvent(task, SwingTask.PROPERTY_DONE, Boolean.FALSE, Boolean.TRUE));
    ...
    private class TaskServicePCL implements PropertyChangeListener
    {
        ...
        public void propertyChange(final PropertyChangeEvent event)
        {
            ...
            {
                List<AbstractTask<?, ?>> oldList = (List<AbstractTask<?, ?>>) event.getOldValue();
                List<AbstractTask<?, ?>> newList = (List<AbstractTask<?, ?>>) event.getNewValue();
    ...
    /**
     * Setzt den ersten der ausgeführenden Tasks und steuert den InputBlocker.
     *
     ...
     */
    private void updateTasks(final List<AbstractTask<?, ?>> oldTasks, final List<AbstractTask<?, ?>> newTasks)
    {
        if (newTasks.isEmpty())
        {
            setForegroundTask(null);
